...
- [Глава 10. Логика и переключатели](#Глава-10-Логика-и-переключатели)
- [Глава 11. Логические вентили](#Глава-11-Логические-вентили)
- [Глава 12. Двоичный сумматор](#Глава-12-Двоичный-сумматор)
- [Глава 13. А как насчет вычитания?](#Глава-13-А-как-насчет-вычитания)
- [Глава 14. Обратная связь и триггеры](#Глава-14-Обратная-связь-и-триггеры)
...
При сложении двух двоичных цифр A и B бит суммы реализуется как **XOR** (Искл-ИЛИ), а бит переноса — как **AND**.
...
Начнем решение с крайнего правого столбца. 
* Сначала мы замечаем, что `6` больше `3`, поэтому нам нужно занять `1` у `5`, а затем вычесть `6` из `13`, в результате чего получается `7`. 
* Мы помним, что заняли `1` у `5`, поэтому вместо `5` имеем `4`, что меньше `7`, поэтому занимаем `1` у `2`, вычитаем `7` из `14` и получаем `7`. 
...
Алгоритм перевода числа в дополнительный код - инвертирование вычитаемого и прибавление к нему 1: 
инвертируем вычитаемое 176 и прибавляем 1:
 176 => !10110000 => 01001111
 1 => 00000001 
результат доп. код = 01001111 + 00000001 = 01010000 
...
fn main() {
    ...
    println!("Выполняем вычисление: {a} ({:08b}) + !{b} ({:08b}) = {} ({:08b})", 
      a, 
      -(b as i8),
      a as i8 + (-(b as i8)),
      a as i8 + (-(b as i8)));
...
Волшебство наблюдается, когда вы размыкаете верхний переключатель. Поскольку выход вентиля ИЛИ-НЕ — 0 (ИЛИ-НЕ это `NOR — !(A || B)`), если один из входов — 1, выход левого вентиля ИЛИ-НЕ не изменяется, и лампочка не гаснет.
...
<div class="sim-wrapper" data-circuit-id="3">
  ...
  <iframe 
      ...
      id="3"
      class="sim-iframe"
      src="./circuitjs/circuit-frame.html?running=0&editable=1&usResistors=0&whiteBackground=1&startCircuit=/3_trigger.txt"
      loading="lazy">
  </iframe>
</div>
...
<div class="sim-wrapper" data-circuit-id="162">
  <button class="sim-fullscreen-btn" data-circuit-id="162">⛶</button>
  <iframe 
      id="162"
      data-circuit-id="162"
      class="sim-iframe"
