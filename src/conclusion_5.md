...
`gcc -std=c99 -Wall -Wextra -Wformat -Werror -Wformat=2 -Wformat-security -O0 main.c -o main.out`
...
void *memset_fast(void *dest, int c, size_t n) {
    ...
    */
    ...
    uint32_t *p32 = (uint32_t*)dest;// соответсвенно передвигатьсяпо массиву будем тоже размерами слов
    ...
    char *p8 = (char*)dest + words * 4;// снаходим позицию для начала записи
