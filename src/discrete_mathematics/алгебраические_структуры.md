...
поле
решётки
Алгебраическая структура - булевы алгебры
```
...
// rayon_logs
    ...
    // Генерируем логи для потоков
    let logs_per_thread: Vec<rayon_logs::Logs> = (0..4).map(|i| {
        let entries = (0..5).map(|j| rayon_logs::LogEntry { t: i*10+j, msg: format!("Thread {} log {}", i, j) }).collect();
        rayon_logs::Logs::from_vec(entries)
    }).collect();

    // Объединяем параллельно
    let merged = rayon_logs::parallel_merge(logs_per_thread.clone());
    println!("Merged logs (параллельно):");
    for e in merged.current() {
        println!("t={} msg={}", e.t, e.msg);
    }

    // Создаем undo для каждого потока
    let (_merged, undos) = rayon_logs::parallel_merge_with_undo(logs_per_thread.clone());

    // Применяем undo к merged логам
    let after_undo = undos.into_par_iter().reduce(|| merged.clone(), |acc, u| acc.merge(&u.inverse()));

    println!("\nПосле применения undo всех потоков (должно быть пусто):");
    for e in after_undo.current() {
        println!("t={} msg={}", e.t, e.msg);
    }

    assert!(after_undo.current().is_empty());
    println!("\nUndo всех потоков прошел успешно ✅");
...
Пример: фильтры как булева алгебра
